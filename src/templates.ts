...
function workflow(id: string, name: string, steps: NonEmptyArray<TemplateWorkflowStep>): WorkflowConfig {
  return {
...
function commonAdapters(): Record<string, AdapterConfig> {
  return {
...
function genericPlanReview(projectName: string): Template {
  const roles = {
    "planner.json": role(
      "planner",
      "Planner",
...
function featurePlanning(projectName: string): Template {
...
export function buildTemplate(templateId: TemplateId, projectName: string): Template {
