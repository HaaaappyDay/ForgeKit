...
import { buildTemplate, TEMPLATE_IDS } from "./templates.js";
...
async function chooseTemplate(current, yes) {
  if (current) {
    if (!TEMPLATE_IDS.includes(current)) {
  ...
  try {
    ...
    if (!TEMPLATE_IDS.includes(selected)) {
...
function validateGenerated(schemaId, value, path) {
...
async function writeGeneratedJson(root, relativePath, value, schemaId) {
  const target = join(root, relativePath);
  await validateGenerated(schemaId, value, target);
  await writeJsonFile(target, value);
}

async function writeGitkeep(path) {
...
export async function runInitCommand(args, cwd = process.cwd()) {
  ...
  for (const [file, value] of Object.entries(template.examples)) {
    const schemaId = file.startsWith("roles/") ? "forgekit.role.v1" : "forgekit.workflow.v1";
