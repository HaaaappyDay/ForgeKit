...
function decisions(handoff) {
  return Array.isArray(handoff.decisions)
    ? handoff.decisions.map((item) => item?.decision).filter((item) => typeof item === "string")
    : [];
}

function summaryPath(projectRoot, runId) {
  return join(projectRoot, ".forgekit/runs", runId, "context/workflow-summary.json");
}

async function validateWorkflowSummary(summary) {
...
export function createWorkflowSummary(run, workflow) {
  return {
...
export async function writeWorkflowSummary(projectRoot, summary) {
  await validateWorkflowSummary(summary);
  await writeJsonFile(summaryPath(projectRoot, summary.run_id), summary);
}

export async function readWorkflowSummary(projectRoot, runId) {
  return readJsonFile(summaryPath(projectRoot, runId));
}

export async function updateWorkflowSummary(projectRoot, { run, workflow, stepIndex, handoff, handoffRef }) {
