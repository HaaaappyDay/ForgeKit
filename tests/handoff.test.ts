...
test("parseHandoffFromRaw unwraps handoff envelope from Claude result", () => {
  const raw = JSON.stringify({
    type: "result",
  ...
  assert.equal(parsed.errors.length, 0);
  assert.ok(parsed.handoff);
  assert.equal(parsed.handoff.schema_version, "handoff.v1");
  assert.equal(parsed.handoff.step_id, "step-1");
});

test("parseHandoffFromRaw reads handoff inside malformed envelope", () => {
  const raw = JSON.stringify({
    type: "result",
    result: `{"handoff": ${JSON.stringify(handoff())}`
  });
  const parsed = parseHandoffFromRaw(raw);
  assert.equal(parsed.errors.length, 0);
  assert.ok(parsed.handoff);
