...
async function assertValidJson(schemaId, path) {
...
test("init creates feature-planning template", async () => {
  await withTempProject(async (dir) => {
    ...
    const config = await assertValidJson("forgekit.config.v1", join(dir, ".forgekit/config.json"));
...
test("init creates generic-plan-review template", async () => {
  await withTempProject(async (dir) => {
    ...
    const workflow = await assertValidJson(
