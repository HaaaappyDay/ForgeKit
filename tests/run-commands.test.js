...
async function patchConfig(projectRoot, mutate) {
...
function successfulAdapterScript() {
  return `#!/bin/sh
...
function flakyAdapterScript(stateFile) {
  return `#!/bin/sh
${fakeHandoffShellFunction()}
if [ "$1" = "-p" ]; then
  for last
...
test("run retry appends a new attempt and keeps prior artifacts", async () => {
  await withTempProject(async (dir) => {
    ...
    const retriedRun = await readJsonFile(join(dir, ".forgekit/runs", failedRun.run_id, "run.json"));
